...
class Controller extends \think\Controller {
  ...
  function __construct( Request $request = null ) {
    ...
    // 初始化配置信息
    $this->config = Config::has( 'template' ) ? Config::get( 'template' ) : $this->config;
    // 处理路由参数
    ...
    // 格式化路由的插件位置
    $this->addon = $convert ? strtolower( array_shift( $param ) ) : array_shift( $param );
    $this->action = $convert ? strtolower( array_pop( $param ) ) : array_pop( $param );
    $this->controller = $convert ? strtolower( implode( '', $param ) ) : implode( '', $param );
